...
const getFilteredUsers = () => {
  return users.filter(u => {
    ...
    if (subscriptionFilter !== 'all' && u.role === UserRole.MEMBER) {
      const expired = isSubscriptionExpired(u.subscriptionEndDate);
      matchesSubscription = subscriptionFilter === 'expired' ? expired : !expired;
    }

    // ✅ فلتر المدرب الجديد
    let matchesTrainer = true;
    if (trainerFilter !== 'all' && u.role === UserRole.MEMBER) {
      matchesTrainer = String(u.trainerId) === trainerFilter;
    }

    return matchesSearch && matchesRole && matchesStatus && matchesSubscription && matchesTrainer;
  });
};
        ...
        <div className="p-4 border-b border-zinc-800 grid grid-cols-1 md:grid-cols-5 gap-4 bg-zinc-950/50">
          ...
          <div className="relative">
            ...
            <select
              ...
              onChange={(e) => setSubscriptionFilter(e.target.value as 'all' | 'active' | 'expired')}
              className="w-full bg-black border border-zinc-700 rounded-lg pr-10 pl-4 py-2.5 text-white focus:ring-1 focus:ring-royal-500 outline-none appearance-none"
            >
